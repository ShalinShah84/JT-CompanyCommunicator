...
class SendConfirmationTaskModule extends React.Component<SendConfirmationTaskModuleProps, IStatusState> {
                    ...
                    getConsentSummaries(id).then((response) => {
                        this.setState({
                            ...
                            this.setState({
                                ...
                                const html = this.state.message.summary;
                        ...
                        <Flex className="scrollableContent" gap="gap.small">
                            <Flex.Item size="size.half">
                                <Flex column className="formContentContainer">
                                    ...
                                    {this.state.message.isNormal &&
                                        <>
                                            <h3>{this.localize("Normal")}</h3>
                                            <label>{this.renderNormal()}</label>
                                        </>
                                    }
                                    {this.state.message.isImportant &&
                                        <>
                                            <h3>{this.localize("Important")}</h3>
                                            <label>{this.renderImportant()}</label>
                                        </>
                                    }
                                    {this.state.message.isAcknowledged &&
                                        <>
                                            <h3>{this.localize("Acknowledgement Required")}</h3>
                                            <label>{this.renderAcknowledge()}</label>
                                        </>
                                    }                                    
                                </Flex>
                            </Flex.Item>
    ...
    private renderAcknowledge = () => {
        if (this.state.message.isAcknowledged) {
            return (
                <label>Yes</label>
            )
        } else {
    ...
    private renderNormal = () => {
        if (this.state.message.isNormal) {
            return (
                <label>Yes</label>
            )
        } else {
