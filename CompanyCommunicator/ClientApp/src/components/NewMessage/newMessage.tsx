...
import { getDraftNotification, getTeams, createDraftNotification, updateDraftNotification, searchGroups, getGroups, getAllUsers, verifyGroupAccess, getAppSettings, getChannelConfig, getGroupAssociations } from '../../apis/messageListApi';
...
const colors = ["White", "Gray", "Standard", "Medium Priority", "High Priority",
];
...
export interface IDraftMessage {
    ...
    csvusers: string,
    allUsers: boolean,
    isNormal: boolean, // indicates if the message is normal
    isImportant: boolean, // indicates if the message is important
    isAcknowledged: boolean;
    isUrgent: boolean;    // indicates if the message is urgent
...
export interface formState {
    ...
    individualOptionSelected: boolean,
    teams?: any[],
    groups?: any[],
    color?:string,
    exists?: boolean,
    ...
    selectedMessageBtn: string,
    selectedNormal: boolean, //status of the importance selection on the interface
    selectedImportant: boolean, //status of the importance selection on the interface
    selectedAcknowledge:boolean, // status of the acknowledge 
    selectedUrgent: boolean, //status of the urgent selection on the interface
...
class NewMessage extends React.Component<INewMessageProps, formState> {
    ...
    constructor(props: INewMessageProps) {
        ...
        this.state = {
            ...
            individualOptionSelected: false,
            csvLoaded: "",
            csvError: false,
            csvusers: "",
            individualUsers: "",
            ...
            selectedMessageBtn:"normal",
            selectedNormal: true, //important flag for the msg is false by default
            selectedImportant: false, //important flag for the msg is false by default
            selectedAcknowledge: false,
            selectedUrgent: false, //urgent flag for the msg is false by default
                ...
                if ('id' in params) {
                    ...
                    this.getItem(id).then(() => {
                        ...
                        this.setState({
                            ...
                            selectedNormal: this.state.selectedNormal,
                            selectedImportant: this.state.selectedImportant,
                            selectedUrgent:this.state.selectedUrgent,
    ...
    private getUserData = async () => {
        try {
            ...
            console.log("individualUsers" + response);
            this.setState({
                individualUsers: response.data
            });
        }
        catch (error) {
            return error;
    ...
    private getItem = async (id: number) => {
        try {
            ...
            let selectedMessageBtn = "normal";
            if (draftMessageDetail.isNormal) {
                selectedMessageBtn = "normal";
            }
            else if (draftMessageDetail.isImportant) {
                selectedMessageBtn = "important";
            }
            else if (draftMessageDetail.isUrgent) {
                selectedMessageBtn = "urgent";
            }

            const html = draftMessageDetail.summary;
            const contentBlock = htmlToDraft(html);
            if (contentBlock) {
                const contentState = ContentState.createFromBlockArray(contentBlock.contentBlocks);
                this.editorState = EditorState.createWithContent(convertFromRaw(mdToDraftjs(draftMessageDetail.summary)));
            }
            ...
            this.setState({
                ...
                selectedColor: draftMessageDetail.color,
                selectedSchedule: draftMessageDetail.isScheduled,
                selectedMessageBtn : selectedMessageBtn,
                selectedNormal: draftMessageDetail.isNormal,
                selectedImportant: draftMessageDetail.isImportant,
                selectedAcknowledge: draftMessageDetail.isAcknowledged,
                selectedUrgent: draftMessageDetail.isUrgent,
            ...
            if (draftMessageDetail.buttonTitle && draftMessageDetail.buttonLink && !draftMessageDetail.buttons) {
                ...
                if (draftMessageDetail.buttons !== null) { //if the database value is not null, parse the JSON to create the button objects
                    //let values = JSON.parse(draftMessageDetail.buttons);
                    //var index = values.findIndex(obj => obj.title === "Acknowledge");
                                    ...
                                    <Flex column className="formContentContainer">
                                        ...
                                        <div className="editorTextArea">
                                            ...
                                            <Editor
                                                ...
                                                editorClassName="summaryEditor"
                                                stripPastedStyles={true}
                                                onEditorStateChange={this.onSummaryChanged.bind(this)}
                                                autoComplete="off"
                                            />
                                        </div>
                            ...
                            <Flex className="scrollableContent">
                                <Flex.Item size="size.half">
                                    <Flex column className="formContentContainer">
                                        ...
                                        {/*<h3>Additional Individual</h3>*/}
                                        {/*<Flex className="peopleContainer">*/}
                                        {/*    <PeoplePicker*/}
                                        {/*        showMax={5}*/}
                                        ...
                                        <Flex>
                                            <h3><Checkbox
                                                className="Important"
                                                labelPosition="start"
                                                onClick={this.onAcknowledgeSelected}
                                                label={this.localize("Acknowledgement")}
                                                checked={this.state.selectedAcknowledge}
                                                toggle
                                            /></h3>
                                        </Flex>
                                        <Text size="small" align="start" content="Select this option to acknowledge your message. Recipients will receive the message and will acknowledge the message." />                                        
                                        <h3>Delivery Options</h3>
                                        <Text className="messageAlert" size="small" align="start" content="Select a option for your message. Important option will be marked as IMPORTANT! and will be shown under Teams Activity." />
                                        <Flex gap="gap.large" hAlign="start">
                                                <RadioGroup
                                                    className="radioBtns"
                                                    checkedValue={this.state.selectedMessageBtn}
                                                    onCheckedValueChange={this.onMessageSelected}
                                                    vertical={true}
                                                    items={[
                                                        {
                                                            name: "normal",
                                                            key: "normal",
                                                            value: "normal",
                                                            label: this.localize("Normal"),
                                                            onClick: this.onNormalSelected,
    ...
    private onImportantSelected = () => {
        this.setState({
            selectedImportant: !this.state.selectedImportant,
            selectedUrgent: false,
            selectedNormal:false
        });
    }

    // handler for the urgent message checkbox
    private onAcknowledgeSelected = () => {
        this.setState({ selectedAcknowledge: !this.state.selectedAcknowledge });
        //this.updateCard();
    }

    // handler for the normal message checkbox
    ...
    private onMessageSelected = (event: any, data: any) => {
        this.setState({
            selectedMessageBtn: data.value,
            selectedUrgent: data.value === "urgent" ? true : false,
            selectedImportant: data.value === "important" ? true :false,
            selectedNormal: data.value === "normal" ? true : false
        });
    }
    ...
    private onColorChange = (event: any, itemsData: any) => {
        this.setState({
            selectedColor: itemsData.value,
        })
    }
    ...
    private onSave = () => {
        ...
        console.log("draftToMarkdown" + draftToMarkdown(convertToRaw(this.state.summary.getCurrentContent())));

        const draftMessage: IDraftMessage = {
            id: this.state.messageId,
            title: this.state.title,
            imageLink: this.state.imageLink,
            summary: draftToMarkdown(convertToRaw(this.state.summary.getCurrentContent())),
            ...
            isNormal : this.state.selectedNormal,
            isImportant: this.state.selectedImportant,
            isAcknowledged: this.state.selectedAcknowledge,
            isUrgent:this.state.selectedUrgent,
    ...
    private onTitleChanged = (event: any) => {
        ...
        setCardSummary(this.card, this.state.summary.getCurrentContent());
    ...
    //private onSummaryChanged = (editorState:any) => {
    //    console.log(editorState.getCurrentContent().getPlainText());
    //    this.setState({
    //        summary: editorState.getCurrentContent().getPlainText(),
    //    });
    //};
    ...
    private onSummaryChanged = (editorState: any) => {
        ...
        setCardTitle(this.card, this.state.title);
        setCardImageLink(this.card, this.state.imageLink);
        setCardSummary(this.card, draftToMarkdown(convertToRaw(editorState.getCurrentContent())));
        setCardAuthor(this.card, this.state.author);
        setCardBtns(this.card, this.state.values);
        this.setState({
            summary: editorState,
    ...
    private updateCard = () => {
        ...
        //if (this.state.selectedAcknowledge) {
        //    var textNode = document.createTextNode("Acknowledge");
        //    var buttonTag = document.createElement("button");
        //    buttonTag.className = 'ac-pushButton';
        //    buttonTag.appendChild(textNode);
        //    //document.getElementsByClassName('ac-actionSet')[0].appendChild(pTag);
        //    var firstWrap = document.getElementsByClassName('ac-actionSet')[0];
        //    this.prependEle(firstWrap, buttonTag);
        //}
        const link = this.state.btnLink;
        adaptiveCard.onExecuteAction = function (action:OpenUrlAction) { window.open(link, '_blank'); }
    }
    private prependEle = (parent, child) =>  {
